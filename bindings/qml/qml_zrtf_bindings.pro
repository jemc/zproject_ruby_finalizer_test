...
TARGET = $$qtLibraryTarget(qml_zrtf)
uri = QmlZrtf
...
android {
  # Use a default value assuming the zrtf project sits outside this one
  isEmpty(ZRTF_ROOT) {
    ZRTF_ROOT = $$clean_path($$PWD/../..)
  }
  !exists($$ZRTF_ROOT) {
    error(The ZRTF_ROOT directory does not exist: $$ZRTF_ROOT)
  }
  # Build the zrtf library for android unless it is already built
  !system(bash $$ZRTF_ROOT/builds/qt-android/build.sh) {
    error(Failed to build the zrtf C library with $$ZRTF_ROOT/builds/qt-android/build.sh)
  }
  
  VENDOR_PREFIX = $$ZRTF_ROOT/builds/qt-android/prefix/$(TOOLCHAIN_NAME)
