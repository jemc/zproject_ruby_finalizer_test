...
##
# Build zrtf from local source

(android_build_verify_so "libzrtf.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/zrtf"
    (cp -r ../.. "${cache}/zrtf" && cd "${cache}/zrtf" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/zrtf" && ./autogen.sh \
...
android_build_verify_so "libzrtf.so" "libczmq.so"
